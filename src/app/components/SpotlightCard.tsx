  ...
  return (
    <div className=" mt-6 md:mt-0 md:w-64 h-auto bg-[#FFFFFF] drop-shadow-3xl pb-4 dark:bg-[#3B3E47]">
      <div className="w-full flex items-center justify-center">
        <Image src={`/${props?.deta?.img}`} width="236" height="401" alt="Img" />
      </div>
      ...
      <div className="w-full text-center flex flex-col items-center justify-center gap-4 mt-4">
        <div className="font-poppins font-medium text-black text-[17px] dark:text-white">{props?.deta?.name}</div>
        <div className="font-poppins font-normal text-black text-[14px] dark:text-white">OCT 15 | SUN | 4:30PM</div>
        <div className="font-poppins font-normal text-[#525965] text-[14px] dark:text-[#DFDFDF]">{props?.deta?.address}</div>
        <div className="font-poppins font-medium text-white text-[12px] bg-[#1D1D1F] w-[213px] h-9 flex items-center justify-center hover:bg-white hover:text-black cursor-pointer">{props?.deta?.btn}</div>
      </div>
    </div>
  );
};
